...
void spi_init( uint8_t spi_speed , MSTR_SLV mst_slv  )
{
	...
	SPCR = ( 1<<SPE ) | ( (mst_slv == MASTER) <<MSTR ) | ( (spi_speed & 0x03)<< SPR0  ) | ( 0<<CPOL ) | ( 0<<CPHA ) | ( 0 << DORD )  ;    /* Enable SPI module , controller is master or slave  SPI clock speed = Fosc/ 16 . */     
